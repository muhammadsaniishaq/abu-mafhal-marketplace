    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const productData = {
                ...
                // specifications: formData.specifications.filter(s => s.key && s.value), // Moved to metadata
                // variants: formData.variants, // Moved to metadata
                // sale_start_date: formData.saleStart || null, // Moved to metadata
                // sale_end_date: formData.saleEnd || null, // Moved to metadata
                shipping_weight: parseFloat(formData.weight) || null,
                seo_title: formData.seoTitle,
                seo_description: formData.seoDesc,
                tags: formData.tags.split(',').map(t => t.trim()).filter(Boolean),
                metadata: {
                    metrics: initialData?.metadata?.metrics || {}, // Preserve existing metrics
                    keywords: formData.keywords,
                    barcode: formData.barcode, // Moved here
                    specifications: formData.specifications.filter(s => s.key && s.value), // Moved here
                    variants: formData.variants, // Moved here
                    video: videoUrl, // Moved here
                    // New Metadata Fields
