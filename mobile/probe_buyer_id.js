...
async function probeBuyerId() {
    // Try to insert a row with buyer_id
    const { error } = await supabase
        .from('orders')
        .insert([{ buyer_id: '00000000-0000-0000-0000-000000000000' }]);

    if (error) {
        console.log('Probe Error:', error.message);
    } else {
        console.log('Success! buyer_id is a valid column.');
    }
}
probeBuyerId();
